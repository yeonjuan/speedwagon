import { collectors } from "../collectors/index.js";
import type { Rule } from "./types.js";

export const duplicateInterfaceDeclaration: Rule = {
  id: "duplicate-interface-declaration",
  category: "duplication",
  collectors: [collectors.interfaceDeclaration],
  descriptions: {
    duplicated:
      "'{{type}}' interface is defined identically in {{count}} places",
  },
  check(context, [interfaceDeclaration]) {
    for (const key of interfaceDeclaration.keys()) {
      const collections = interfaceDeclaration.getByKey(key);
      if (collections.length <= 1) {
        continue;
      }
      const displayName = collections[0].displayName;
      context.report({
        descriptionId: "duplicated",
        data: { type: displayName, count: collections.length },
        occurrences: collections.map(({ path, location }) => ({
          path,
          location,
        })),
      });
    }
  },
};
