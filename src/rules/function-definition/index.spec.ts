...
import { createFunctionDefinitionRule } from "./index.js";

describe("FunctionDefinitionRule", () => {
  const collector = createFunctionDefinitionRule({ minOccurrences: 2 });
...
class Formatter {
  ...
  it("respects minOccurrences", async () => {
    const strictRule = createFunctionDefinitionRule({
      minOccurrences: 3,
    });
    const tester = new RuleTester(strictRule);
    const reports = await tester.testSingleFile(`
function first(a) {
  return a + 1;
}
